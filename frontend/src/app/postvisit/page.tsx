...
import { simplifyExplanation, searchMedicines, type MedicineInfo } from '@/lib/api';
              ...
              <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-5">
                ...
                {medicineResults.length > 0 && (
                  <div className="mt-3 space-y-1.5">
                    <p className="text-[10px] text-gray-400 mb-1">{medicineResults.length}건의 약이 검색되었습니다. 약 이름을 눌러 상세 정보를 확인하세요.</p>
                    {medicineResults.map((med) => {
                      const isExpanded = expandedMedicine === med.name;
                      const infoRows: { label: string; value: string; color: string }[] = [
                        { label: '분류', value: med.category, color: 'text-indigo-600' },
                        { label: '성분', value: med.ingredients, color: 'text-emerald-600' },
                        { label: '구분', value: med.type, color: 'text-blue-600' },
                        { label: '보관법', value: med.storage, color: 'text-gray-500' },
                      ].filter((s) => s.value);

                      const docLinks: { label: string; url: string; color: string }[] = [
                        { label: '효능·효과', url: med.eeDocUrl, color: 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100' },
                        { label: '용법·용량', url: med.udDocUrl, color: 'bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-100' },
                        { label: '주의사항', url: med.nbDocUrl, color: 'bg-orange-50 text-orange-700 border-orange-200 hover:bg-orange-100' },
                      ].filter((l) => l.url);
                        ...
                        <div key={med.name} className="rounded-lg border border-gray-100 overflow-hidden">
                          ...
                          >
                            <span className="flex items-center gap-1.5">
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className={isExpanded ? 'text-emerald-500' : 'text-gray-400'}>
                          ...
                          {isExpanded && (
                            <div className="px-3 py-2.5 bg-white border-t border-gray-100 space-y-2">
                              {med.company && (
                                <p className="text-[10px] text-gray-400">제조사: {med.company}</p>
                              )}
                              {infoRows.map((row) => (
                                <div key={row.label}>
                                  <span className={`text-[10px] font-semibold uppercase tracking-wider ${row.color}`}>{row.label}</span>
                                  <p className="text-xs text-gray-600 mt-0.5 leading-relaxed">{row.value}</p>
                                </div>
                              ))}
                              {docLinks.length > 0 && (
                                <div className="pt-1.5 border-t border-gray-50">
                                  <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider block mb-1.5">식약처 상세 문서</span>
                                  <div className="flex flex-wrap gap-1.5">
                                    {docLinks.map((link) => (
                                      <a
                                        ...
                                        className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-md border text-[11px] font-medium transition-colors ${link.color}`}
                                      >
                                        {link.label}
                                        <svg width="10" height="10" viewBox="0 0 12 12" fill="none" className="opacity-60">
                                          <path d="M4.5 2H2.5C1.95 2 1.5 2.45 1.5 3v7c0 .55.45 1 1 1h7c.55 0 1-.45 1-1V8" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" />
                                          <path d="M7 1.5h3.5V5M6 6.5L10.5 2" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
                                        </svg>
                                      </a>
                                    ))}
                                  </div>
